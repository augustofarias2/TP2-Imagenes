...
def detect_patent(filename):
    ...
    cv2.imshow("whitish_pixels", whitish_pixels)
    cv2.waitKey(0)
    ...
    for st in stats:
        x, y, w, h, area = st
        ratio = w / h
        highlighted = cv2.rectangle(highlighted, (x, y), (x + w, y + h), color=(255, 0, 0), thickness=1)
    ...
    cv2.imshow("Componentes Conectados", highlighted)
    cv2.waitKey(0)
    return possible_patents


def detect_characters(patent, filename, V_threshold):
    cv2.imshow("Patente Original", patent)
    cv2.waitKey(0)
...
possible_patents = detect_patent('Patentes\img03.png')
characters = []
for patent in possible_patents:
    
    if len(characters) != 6:
        characters = detect_characters(patent, 'img04.png', 140)
        if len(characters) != 6:
            characters = detect_characters(patent, 'img04.png', 120)
